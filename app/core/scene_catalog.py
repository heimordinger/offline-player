...
from app.core.preview_loader import card_face_local, card_id_from_story, first_local_adv_image
...
class SceneCatalog:
    ...
    def is_section_hub(self, cat: str) -> bool:
        return self._category_mode == "minashigo" and bool(
            is_viewer_section_hub and is_viewer_section_hub(cat)
        )

    def is_card_hub(self, cat: str) -> bool:
        """Character / Summon / Skin：点进后先显示卡面列表。"""
    ...
    def section_cards_with_scenes(self, section: str) -> list[str]:
        if self._category_mode != "minashigo" or not section_cards:
            return []
    ...
    def list_by_category(self, cat: str) -> list[str]:
        ...
        if self.is_section_hub(cat):
            return []
        return [j for j in self._json_list if self._category_of(j) == cat]
    ...
    def category_display_name(self, cat: str) -> str:
        ...
        if self._viewer_index:
            ...
            if cat in names and names[cat]:
                return names[cat]
        if cat in self._category_name_cache:
            return self._category_name_cache[cat]
        if cat == MAIN_STORY:
            return "メインストーリー"
        if cat == SIDE_STORY:
            return "サイドストーリー"
    ...
    def category_caption(self, cat: str) -> str:
        ...
        if cat in self._category_counts:
            return f"{self._category_counts[cat]} 个场景"
        return f"{len(self.list_by_category(cat))} 个场景"
    ...
    def category_icon(self, cat: str) -> str | None:
        ...
        if cat == CUSTOM_CATEGORY:
            ...
            if not os.path.exists(thumb):
                thumb = os.path.join(thumb_root, safe + ".jpg")
    ...
    def scene_list_thumb_paths(
        ...
        """场景列表缩略图：默认卡面，无卡面则用第一张 CG；悬浮优先 CG。"""
